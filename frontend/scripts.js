...
let socket;
function initSocket() {
  socket = io(API_BASE); // requires socket.io client script included in HTML
  socket.on("connect", () => console.log("Socket connected:", socket.id));
  // When server emits updated leaderboard
  socket.on("leaderboard-updated", (leaderboard) => {
    // update local UI immediately when server pushes new data
    renderLeaderboardFromServer(leaderboard);
  });
}
// call initSocket after DOM is ready
initSocket();

// ===== helper to render leaderboard (server version) =====
function renderLeaderboardFromServer(list) {
  leaderboardEntries.innerHTML = '';
...
// ===== fetch leaderboard on demand (fallback) =====
async function loadLeaderboardFromServer() {
  try {
    const res = await fetch(`${API_BASE}/api/players/leaderboard`);
    if (!res.ok) throw new Error("Leaderboard fetch failed");
    const data = await res.json();
    renderLeaderboardFromServer(data);
  } catch (err) {
    console.error(err);
  }
}

// call `loadLeaderboardFromServer` when leaderboard modal opens instead of updateLeaderboardDisplay
leaderboardButton.removeEventListener('click', /* your old handler */);
leaderboardButton.addEventListener('click', function(event) {
  event.stopPropagation();
  playClickSound();
...
// ===== register player to backend (call when user registers) =====
async function registerPlayerOnServer(name) {
  try {
...
// adjust your existing register button handler to call registerPlayerOnServer
registerButton.addEventListener('click', async function() {
  ...
  // register on server and save id
  const player = await registerPlayerOnServer(name);
...
async function uploadImagesToServer(jhatuFile, ganduFile) {
  const playerId = localStorage.getItem("playerId");
  if (!playerId) return;
  try {
    const fd = new FormData();
    if (jhatuFile) fd.append("jhatuImage", jhatuFile);
    if (ganduFile) fd.append("ganduImage", ganduFile);
    const res = await fetch(`${API_BASE}/api/players/upload/${playerId}`, {
      method: "POST",
      body: fd
    });
...
// Hook file input change event to send file to backend (example for jhatu)
jhatuUpload.addEventListener('change', function(e) {
  if (e.target.files && e.target.files[0]) {
    // show immediate preview as you already do (FileReader)
    const file = e.target.files[0];
    uploadImagesToServer(file, null); // send to backend
  }
});
ganduUpload.addEventListener('change', function(e) {
  if (e.target.files && e.target.files[0]) {
    const file = e.target.files[0];
    uploadImagesToServer(null, file);
  }
});

// ===== send score/stats on game over =====
async function pushStatsToServerOnGameOver() {
  const playerId = localStorage.getItem("playerId");
  if (!playerId) return;
  try {
    const payload = {
      ...
      totalGames: 1,
      bestStreak: score,
      ganduHits: ganduHits || 0,
      totalTimePlayed: Math.floor((Date.now() - gameStartTime)/1000)
    };
    await fetch(`${API_BASE}/api/players/update/${playerId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });
    // Optionally you can load leaderboard or rely on socket event to update UI
  } catch (err) {
    console.error("failed to push stats", err);
  }
}
